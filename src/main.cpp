...
uint256 hashGenesisBlock("0x49ecfc5154e942122f8bf35b83b45b25ceba54ebf7d27b7939601d616ace216b");
...
const string strMessageMagic = "LTCCASH Signed Message:\n";
...
{
    // LTCCASH: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // LTCCASH: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "ltccash genesis block";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1500743097;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2486896;
...
unsigned char pchMessageStart[4] = { 0xd2, 0xaa, 0xab, 0xeb };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LTCCASHMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LTCCASHMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LTCCASHMiner(CWallet *pwallet)
{
    printf("LTCCASHMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("LTCCASH-miner");
    ...
    try { loop {
        ...
        printf("Running LTCCASHMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("LTCCASHMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LTCCASHMiner, pwallet));
}
