...
// Copyright (c) 2011-2012 LTCCASH Developers
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #LTCCASHTEST0\r");
            Send(hSocket, "WHO #LTCCASHTEST0\r");
        } else {
            // randomly join #LTCCASH00-#LTCCASH99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #LTCCASH%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #LTCCASH%02d\r", channel_number).c_str());
        }
